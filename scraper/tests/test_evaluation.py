...
# ═══════════════════════ ce qui accompagne la fiche, et pourquoi il le faut

def test_le_calendrier_resolu_accompagne_la_fiche():
    """Le corpus connaît des **dates** ; le modèle rend des « mercredis ».

    Sans cette conversion, un spectacle hebdomadaire comptait MANQUÉ à chaque
    mesure : la sortie approuvée porte les trois mercredis que le site a
    calculés, la fiche n'annonce aucune date. On reprochait à l'étage 6 un
    calcul qui a lieu deux étages plus loin.
    """
    out = extract_page(
        "https://theatre.exemple.fr/le-petit-prince",
        PAGE,
    ...
    assert out["fiche"]["resolvedDates"] == ["2026-08-05", "2026-08-12", "2026-08-19"]


def test_la_prose_du_modele_reste_a_cote_du_calendrier():
    """Les deux voyagent, et ils ne disent pas la même chose : `weekdays` dit ce
    que le modèle a **lu**, `resolvedDates` ce que le site aurait **enregistré**.
    Remplacer l'un par l'autre ferait perdre la pièce à conviction."""
    out = extract_page(
        "https://exemple.fr/x", PAGE, provider=FakeProvider(JUSTE), config=FakeConfig(), log=None
    )

    assert out["fiche"]["weekdays"] == ["mercredi"]
    assert out["fiche"]["dates"] == []


def test_sans_recurrence_le_calendrier_reste_vide():
    """Vide veut dire « tous les jours de la plage » — la convention du site, et
    donc un accord avec une sortie publiée sans calendrier précis. Ce n'est pas
    une absence de réponse."""
    out = extract_page(
        "https://exemple.fr/x",
        PAGE,
        provider=FakeProvider(replace(JUSTE, weekdays=())),
        config=FakeConfig(),
        log=None,
    )

    assert out["fiche"]["resolvedDates"] == []


def test_le_calendrier_resolu_vient_de_la_fonction_de_production():
    """L'invariant, le même qu'ailleurs dans ce module : on mesure la fonction,
    pas une relecture de ses règles. Si `schedule.resolve` change, ce relevé
    change avec lui — et c'est ce qu'on veut d'un banc."""
    from sortiesbot.schedule import resolve

    attendu = list(resolve(JUSTE.date_start, JUSTE.date_end, JUSTE.weekdays, JUSTE.dates, []).dates)

    assert resolved_days(JUSTE, []) == attendu


def test_les_categories_du_site_partent_au_modele():
    """Le prompt demande de choisir dans une liste. La lui refuser faisait
    compter faux, à chaque fiche, un champ qu'on empêchait de remplir — le banc
    mesurait alors un appel qui n'existe pas en production."""
    provider = FakeProvider(JUSTE)
    ...
    extract_page(
        ...
        config=FakeConfig(),
        log=None,
        categories=["Spectacle", "Atelier"],
    )

    assert provider.seen["categories"] == ["Atelier", "Spectacle"]
