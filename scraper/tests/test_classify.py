...
def test_deux_titres_ne_suffisent_pas_a_faire_un_agenda():
    ...
    # Deux titres : le JSON-LD ne tranche pas, on retombe sur les liens.
    verdict = classify(page(ld(seances) + liens(2)), URL)
    assert verdict.kind == SORTIE
    assert verdict.signal == "liens"
...
def test_un_json_ld_illisible_ne_fait_pas_echouer_le_classement():
    ...
    assert classify(html, URL).kind == AGENDA
...
def test_beaucoup_de_liens_trahissent_une_liste():
    verdict = classify(page(liens(30)), URL)
    assert (verdict.kind, verdict.signal, verdict.confidence) == (AGENDA, "liens", "probable")


def test_une_page_qui_ne_mene_nulle_part_est_la_sortie():
    verdict = classify(page("<h1>Le Petit Prince</h1>" + liens(2)), URL)
    assert (verdict.kind, verdict.signal) == (SORTIE, "liens")


def test_entre_les_deux_le_classifieur_ne_tranche_pas():
    """« Inconnu » est une réponse : l'orchestrateur sait déjà quoi en faire."""
    verdict = classify(page(liens(6)), URL)
    assert (verdict.kind, verdict.confidence) == (INCONNU, "faible")
